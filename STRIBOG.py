...
class STRIBOG(HEXANDBINARY):
    ...
    def HASHVALUE(self, M1, check = 512):
        '''
        check - 512 или 256, по дефолту 512
        M1 - сообщение обычное (ASCII)
        возвращается значение в 16ичном виде
        '''
        h = '0'*512
        N = '0'*512
        EPSILON = '0'*512
        ...
        M1b = ''.join(format(ord(c),'08b') for c in M1)
        ...
        while (len(M1b) >= 512):
            ...
            h = self.g_N(N, h, m)
            N = self.Vec_512(self.add_mod2512(N, format(512, '0512b')))
            EPSILON = self.Vec_512(self.add_mod2512(EPSILON, m))
            M1b = m_strih
        ...
        N = self.Vec_512(self.add_mod2512(N, format(len(M1b), '0512b')))
        EPSILON = self.Vec_512(self.add_mod2512(EPSILON, M1b_padded))
...
if __name__ == "__main__":
    '''
    s = STRIBOG()
    msg_in_hex_example = '323130393837363534333231303938373635343332313039383736353433323130393837363534333231303938373635343332313039383736353433323130'
    msg = '210987654321098765432109876543210987654321098765432109876543210'
    t1 = s.HASHVALUE(msg)
    t2 = '486f64c1917879417fef082b3381a4e211c324f074654c38823a7b76f830ad00fa1fbae42b1285c0352f227524bc9ab16254288dd6863dccd5b9f54a1ad0541b'
    print(t1==t2)
    '''
    pass
